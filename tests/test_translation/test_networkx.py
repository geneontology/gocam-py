...
def test_networkx_roundtrip_compatibility(get_model, translator):
    ...
    # In networkx<3.6, if `edges` is not provided it defaults to looking for a "links" attribute
    # in the graph data. In networkx>=3.6, the `edges` parameter has a default value of "edges", but
    # we set it explicitly here to ensure compatibility with both versions.
    reconstructed_graph = nx.node_link_graph(data, edges="edges")
...
def test_model_evidence_collections_basic_568b0f9600000284(get_model, translator):
    ...
    evidence_attrs = [
        ...
        if any(
            suffix in key
            for suffix in ["_has_reference", "_assessed_by", "_contributors"]
        )
    ]
    assert len(evidence_attrs) > 0, "Should have evidence collection attributes"


def test_model_gene_product_evidence_collections_568b0f9600000284(
